...
static uint32_t s3c_gpio_read(void *opaque, target_phys_addr_t addr)
{
    ...
    /* Per bank registers */
    case S3C_GPCON:
        llhwl_report_event_data_put(LLHWL_EVENT_GPCON_READ, &bank);
        llhwl_report_event(LLHWL_EVENT_GPCON_READ, s);
        return s->bank[bank].con;
    case S3C_GPDAT:
        llhwl_report_event_data_put(LLHWL_EVENT_GPDAT_READ, &bank);
        llhwl_report_event(LLHWL_EVENT_GPDAT_READ, s);
